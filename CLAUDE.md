...
- Conventional commits enforced via cocogitto (`cog check`) and git commit-msg hook
- CI via [barryw/woodpecker-release](https://github.com/barryw/woodpecker-release) config service — template in `.woodpecker/woodpecker-template.yaml`
- Releases automated: `feat:` → minor bump, `fix:` → patch, `feat!:` → major. Bump commits include `[skip ci]`.
- README auto-generated by terraform-docs between `<!-- BEGIN_TF_DOCS -->` markers (GitHub Action on PRs)
